...
export default function TvDisplay() {
  ...
  // by its own remote — no phone, no trainer login required.
  useEffect(() => {
    if (!token) return
    let lastPress = 0
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      const now = Date.now()
      if (now - lastPress < 400) return // debounce rapid/held button presses

      let direction: 'next' | 'previous' | null = null
      if (e.key === 'ArrowRight' || e.key === 'Enter') direction = 'next'
