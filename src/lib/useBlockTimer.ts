...
export function useBlockTimer(block: DisplayBlock | null, startedAt: string | null): TimerState {
  ...
  if ((block.type === 'interval' || block.type === 'circuit') && block.workSeconds && block.restSeconds) {
    if (notStarted) {
      const roundLabel = block.rounds ? `${block.rounds} ROUNDS` : ''
      return { primary: formatClock(block.workSeconds), label: `READY · ${roundLabel}`.trim(), phase: 'neutral' }
    }
  ...
  if ((block.type === 'emom' || block.type === 'amrap') && block.timeCapSeconds) {
    if (notStarted) {
      return { primary: formatClock(block.timeCapSeconds), label: 'READY', phase: 'neutral' }
    }
  ...
  // warmup / straight_sets / pyramid / anything without explicit timing.
  if (notStarted) {
    return { primary: '00:00', label: 'READY', phase: 'neutral' }
  }
  return {
